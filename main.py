...
while True:
    ...
    """)
    numb = int(input("Введите номер действия: "))  # Получаем ввод пользователя для выбора действия.
    ...
    # Обработка действия 3: Добавление новой записи.
        ...
        id = int(input("Введите номер рыбы: "))  # Запрашиваем номер записи.
        exists = False  # Проверка, существует ли запись с таким `id`.
        for fish in data:  # Перебираем записи для проверки.
            if fish['id'] == id:
                exists = True  # Если запись с таким `id` существует, устанавливаем `exists` в True.
                break
        if exists:  # Если запись уже есть, выводим сообщение.
            print("Такой номер рыбы уже есть.")
        else:  # Если записи нет, запрашиваем данные для новой записи.
            ...
            sub_type_count = float(input("Введите количество подвидов этой рыбы: "))  
